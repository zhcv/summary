...
CPBD is a perceptual-based no-reference objective image sharpness metric based 
on the cumulative probability of blur detection developed at the Image, Video 
and Usability Laboratory of Arizona State University.

[The metric] is based on the study of human blur perception for varying contrast 
values. The metric utilizes a probabilistic model to estimate the probability 
of detecting blur at each edge in the image, and then the information is pooled 
by computing the cumulative probability of blur detection (CPBD).

This software is a Python port of the reference MATLAB implementation. 
To approximate the behaviour of MATLAB’s proprietary implementation of the Sobel 
operator, it uses an implementation inspired by GNU Octave. 

[References]
CPBD is described in detail in the following papers:

[1] N. D. Narvekar and L. J. Karam, “A No-Reference Image Blur Metric Based on 
    the Cumulative Probability of Blur Detection (CPBD),” in IEEE Transactions 
    on Image Processing, vol. 20, no. 9, pp. 2678-2683, Sept. 2011.
[2] N. D. Narvekar and L. J. Karam, “An Improved No-Reference Sharpness Metric 
    Based on the Probability of Blur Detection,” International Workshop on Video 
    Processing and Quality Metrics for Consumer Electronics (VPQM), January 2010, 
    http://www.vpqm.org (pdf)
[3] N. D. Narvekar and L. J. Karam, “A no-reference perceptual image sharpness 
    metric based on a cumulative probability of blur detection,” 2009 
    International Workshop on Quality of Multimedia Experience, San Diego, CA, 
    2009, pp. 87-91.
"""
#===============================================================================
...
def sobel(image):
    ...
    Inspired by the [Octave implementation]
    https://sourceforge.net/p/octave/image/ci/default/tree/inst/edge.m#l196
    """
...
def _simple_thinning(strength):
    ...
    Inspired by the [Octave implementation]
    https://sourceforge.net/p/octave/image/ci/default/tree/inst/edge.m#l512.
    """
    ...
    for row in range(img_height):
        for col in range(img_width):
            if gradient_x[row, col] != 0:
                edge_angles[row, col] = atan2(gradient_y[row, col], 
                        gradient_x[row, col]) * (180 / pi)
        ...
        for row in range(1, img_height - 1):
            for col in range(1, img_width - 1):
                if edges[row, col] == 1:
                    ...
                    if quantized_angles[row, col] == 180 or \
                        ...
                        for margin in range(100 + 1):
                            ...
                            if outer_border < 0 or \
                                (image[row, outer_border] - image[row, inner_border]) <= 0:
                                break
...
if __name__ == '__main__':
    ...
    """
    root = argv[1]
    imgs = [f for f in os.listdir(root) if f.endswith(('jpg', 'jpeg', 'png'))]
    for img in imgs:
        imgname = os.path.join(root, img)
        input_image = imread(imgname, mode='L')
        sharpness = compute(input_image)
        print('CPBD sharpness for %s %f' % (img, sharpness))
