...
# fork https://www.kaggle.com/olegtrott/the-perfect-score-script

# INPUT / OUTPUT

def read_patient_ids():
    with open('stage2_sample_submission.csv') as f:
        lines = f.readlines()[1:]
...
def write_submit_files():
    ...
    for i, chunk in enumerate(chunks):
        ...
        write_submit(patient_ids, probs, 'submit_Bowl_2017/submission_%02d.csv' % i)
        if i < len(scores):
            update_labels(labels, chunk, t, scores[i])
            if i+1 == len(chunks):
                write_submit(patient_ids, labels, 'submit_Bowl_2017/submission_fin.csv')
